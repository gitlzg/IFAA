...
##' \deqn{\log\bigg(\frac{\mathcal{Y}_i^k}{\mathcal{Y}_i^{K+1}}\bigg)|\mathcal{Y}_i^k>0,\mathcal{Y}_i^{K+1}>0=\alpha^{0k}+\mathcal{X}_i^T\alpha^k+\epsilon_i^k,\hspace{0.2cm}k=1,...,K,}
##' where \eqn{\mathcal{Y}_i^k} is the AA of taxa \eqn{k} in subject \eqn{i} in the entire 
##' ecosystem. \eqn{\mathcal{Y}_i^{K+1}} is the reference taxon (specified by user). 
##' \eqn{\mathcal{X}_i} is the covariate matrix for all covariates including confounders.
##' \eqn{\alpha^k} is the regression coefficients along with their 95% confidence intervals that will be estimated by the `MZILN()` function. 
##' High-dimensional \eqn{X_i} is handled by regularization.
##' 
